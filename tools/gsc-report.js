/*
 * Informe de Search Console para EmpiezaLibros. Solo lectura.
 *
 ...
 * vive fuera del repo, en C:\Users\marti\.empiezalibros-secrets\gsc-service-account.json.
 *
 * Da dos cosas:
 ...
 *       node tools/gsc-report.js --index    (solo indexación, ~100 llamadas, lento)
 */
...
const KEY_PATH = 'C:/Users/marti/.empiezalibros-secrets/gsc-service-account.json';
const SITE = 'sc-domain:empiezalibros.es';
...
async function indexation(token) {
  ...
  if (notIndexed.length) {
    ...
    notIndexed.forEach(r => console.log(`  [${r.state}] ${r.url.replace('https://empiezalibros.es', '')}`));
  }
}
